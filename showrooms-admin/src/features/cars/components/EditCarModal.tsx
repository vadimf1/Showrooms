...
import { updateCar, getAllCarModels, getCarTrimsForModel } from '../api/cars.api';
import { Car, CarModel, CarStatus, CarTrim } from '../model/car.types';
...
export default function EditCarModal({ car, onClose, onSaved, onOpenCatalog, refreshSignal }: Props) {
  ...
  const [form, setForm] = useState<Form>({
    makeFilter:    car.trim.car_model.make,
    carModelId:    car.trim.car_model.id,
    trimId:        car.trim.id,
    vin:           car.vin,
    colorName:     car.color_name,
    colorHex:      car.color_hex,
    mileage:       String(car.mileage),
    purchasePrice: String(car.purchase_price),
    salePrice:     String(car.sale_price),
    status:        car.status,
    showroomId:    car.showroom?.id ?? '',
  ...
  useEffect(() => { if (refreshSignal) fetchModels(); }, [refreshSignal]);
        ...
        <div className="modal-header">
          <div>
            ...
            <p className="modal-sub">
              {car.trim.car_model.make} {car.trim.car_model.model} · {car.vin}
            </p>
          </div>
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            ...
            <label className="form-field">
              ...
              <Combobox
                ...
                onChange={v => set({ carModelId: v, trimId: '' })}
          ...
          <div className="color-presets">
            ...
            <input type="color" className="color-picker-input" value={form.colorHex || '#cccccc'}
              onChange={e => set({ colorHex: e.target.value, colorName: '' })} />
          </div>
          <div className="form-row" style={{ marginTop: 10 }}>
            <label className="form-field">
              <span>Название цвета</span>
              <input className="form-input" value={form.colorName}
                onChange={e => set({ colorName: e.target.value })} placeholder="Pearl White" />
            </label>
            <label className="form-field">
              <span>Hex</span>
              <input className="form-input mono" value={form.colorHex}
                onChange={e => set({ colorHex: e.target.value })} placeholder="#cccccc" />
            </label>
          </div>
          ...
          <div className="form-row">
            <label className="form-field">
              ...
              <input className="form-input" type="number" min="0" value={form.purchasePrice}
          ...
          <div className="form-row">
            ...
            <label className="form-field">
              <span>Дилер</span>
              <select className="form-select" value={form.dealerId}
